export function handleImageScrollEffectRight() {
    const sections = document.querySelectorAll('.scroll-section-right');

    sections.forEach((section) => {
        const imageContainer = section.querySelector('.image-container-right');
        const image = imageContainer?.querySelector('img');
        const text = section.querySelector('.text-container-right');
        ...
        if (textProgress < 1) {
            ...
            image.style.transform = `translateX(-30%)`;
        }
    });
}

window.addEventListener('scroll', handleImageScrollEffectRight);
window.addEventListener('load', handleImageScrollEffectRight);